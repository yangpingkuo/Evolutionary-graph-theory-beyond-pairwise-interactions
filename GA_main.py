...
def rastigin(G, T, s):
    ...
    return np.hstack([best_array[:,None], mean_array[:,None]])
    #return np.hstack([best_array[:,None], mean_array[:,None], traj, var])


if __name__ == "__main__":
    
    #T_epi = int(sys.argv[1])
    ...
    np.savetxt(out_path, results.mean(0))
