    ...
    def tune(self, target, std = 10, T_max = 10000):
        ...
        while t < T_max * len(self.g):
            ...
            if t % len(self.g) == 0:
                ...
                print(t // len(self.g), std, nx.degree_assortativity_coefficient(self.g), self.n_trans)
        ...
        self._check_connectivity()
        print("Final fraction of triangles:", nx.transitivity(self.G))

if __name__ == "__main__":
    sampler = Sampler(nx.read_edgelist(sys.argv[1]) )
    print("Iteration\t Degree standard deviation\t Fraction of triangles")
    sampler.tune(float(sys.argv[2]))
    nx.write_edgelist(sampler.G, sys.argv[3], data = False)
    #print(*sys.argv)
