...
class Sampler(object):
    ...
    def sample(self, n_trails = 100):
        ...
        assert n_e > 1
        idx = np.array(list(self.dtype2idx.keys()))

        for _ in range(n_trails):
            d = idx[int(random.random() > np.exp(-100 * idx[0]) / sum( np.exp(-100 * idx) ) )]                   
            candidates = self.dtype2idx[d]
            r1 = candidates[int(random.random() * len(candidates))]

            e1 = self.edgelist[r1]
            d1 = np.array([self.g.degree(e1[0]), self.g.degree(e1[1]) ] )
            
            d = d1[int(random.random() > (1 / d1[1] / sum(1 / d1) ) )]
            candidates = self.dtype2idx[d]
    ...
    def swap(self, r1, r2):
        ...
        #print([self.g.degree(e1[0]), self.g.degree(e1[1]), self.g.degree(e2[0]), self.g.degree(e2[1]) ] )
